...
def get_cache_file(cache_key):
    ...
    if prompts is None:
        print(f'adding {cache_key}.json to the cache')
        # build file name from cache key
        file_name = f'{cache_key}.json'
    
        # Construct the full file path to your JSON file
        file_path = os.path.join(settings.BASE_DIR, 'back-end', 'backend_function_calls', 'static', file_name)
        ...
        try:
            ...
            print(f'{cache_key}.json is now in the cache')
